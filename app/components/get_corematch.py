...
class GetCoreMatch():
        ...
        with open(path, "r", encoding="utf8") as file:
            while True:
                ...
                if 'CI server version:' in line:
                    ...
                    # Old clients logged `release-XX.XX-N-BUILD`; current clients
                    # already include `shipping`. Inserting it twice makes Riot's
                    # content service reject the request with UNKNOWN_CONTENT_VERSION.
                    if '-shipping-' in version:
                        return version
    ...
    def get_latest_season_id(self):
        ...
        if not seasons:
            error = self.content.get("errorCode", "missing Seasons")
            message = self.content.get("message", "")
            raise RuntimeError(
                f"Riot content service returned no seasons ({error}: {message})"
            )
    ...
    def get_game_state(self):
        for presence in self.presence():
            if presence['puuid'] == self.puuid:
                ...
                # Recent clients nest this value under matchPresenceData.
                # Older clients exposed it directly in the presence payload.
                match_presence = private.get("matchPresenceData", {})
                state = private.get("sessionLoopState") or match_presence.get("sessionLoopState")
                if state:
                    return state
                raise RuntimeError("VALORANT presence does not include a match state yet")

        raise RuntimeError("Could not find the local player in VALORANT presence data")
    ...
    def fetch_match_details(self):
        ...
        if game_state != "INGAME":
            raise RuntimeError(
                f"VALORANT is in {game_state}. Start a custom match and enter the map before calling /start_match."
            )
